...
var index = require('./index-Bqh2AaI-.cjs');
...
var require$$0$3__namespace = /*#__PURE__*/_interopNamespaceDefault(require$$0$3);
...
class ExternalChunk {
    ...
    getFileName() {
        ...
        return (this.fileName =
            (typeof paths === 'function' ? paths(this.id) : paths[this.id]) ||
                (this.renormalizeRenderPath ? index.normalize(path$n.relative(this.inputBase, this.id)) : this.id));
    }
    ...
    getImportPath(importer) {
        return escapeId(this.renormalizeRenderPath
            ? index.getImportPath(importer, this.getFileName(), this.options.format === 'amd', false)
            : this.getFileName());
    }
}
...
class Variable extends ExpressionEntity {
    ...
    addUsedPlace(usedPlace) {
        const isFunctionCall = usedPlace.parent.type === index.CallExpression &&
            usedPlace.parent.callee === usedPlace;
        if (!isFunctionCall && usedPlace.parent.type !== index.ExportDefaultDeclaration) {
...
class ExternalModule {
    constructor(options, id, moduleSideEffects, meta, renormalizeRenderPath, attributes) {
        ...
        this.info = {
            ...
            attributes,
            code: null,
            dynamicallyImportedIdResolutions: index.EMPTY_ARRAY,
            dynamicallyImportedIds: index.EMPTY_ARRAY,
            ...
            id,
            implicitlyLoadedAfterOneOf: index.EMPTY_ARRAY,
            implicitlyLoadedBefore: index.EMPTY_ARRAY,
            importedIdResolutions: index.EMPTY_ARRAY,
            importedIds: index.EMPTY_ARRAY,
            get importers() {
    ...
    warnUnusedImports() {
        ...
        this.options.onLog(index.LOGLEVEL_WARN, index.logUnusedExternalImports(this.id, unused, importersArray));
    }
}
...
function getMatcherString(id, resolutionBase) {
    if (require$$0$4.isAbsolute(id) || id.startsWith('**')) {
        return normalizePath(id);
    }
    // resolve('') is valid and will default to process.cwd()
    const basePath = normalizePath(require$$0$4.resolve(''))
    ...
    return require$$0$4.posix.join(basePath, normalizePath(id));
}
    ...
    parseNode(esTreeNode) {
        for (const [key, value] of Object.entries(esTreeNode)) {
            ...
            if (key.charCodeAt(0) === 95 /* _ */) {
                if (key === index.ANNOTATION_KEY) {
                    this.annotations = value;
                }
                else if (key === index.INVALID_ANNOTATION_KEY) {
...
class LocalVariable extends Variable {
    ...
    deoptimizePath(path) {
        ...
        if (path.length === 0) {
            ...
            this.expressionsToBeDeoptimized = index.EMPTY_ARRAY;
            for (const expression of expressionsToBeDeoptimized) {
                expression.deoptimizeCache();
            }
...
class Identifier extends NodeBase {
    ...
    render(code, { snippets: { getPropertyAccess }, useOriginalName }, { renderedParentType, isCalleeOfRenderedParent, isShorthandProperty } = index.BLANK) {
        if (this.variable) {
            ...
            if (name === 'eval' &&
                renderedParentType === index.CallExpression &&
    ...
    disallowImportReassignment() {
        return this.scope.context.error(index.logIllegalImportReassignment(this.name, this.scope.context.module.id), this.start);
    }
    applyDeoptimizations() {
        this.deoptimized = true;
        if (this.variable instanceof LocalVariable) {
...
const NO_INTERACTIONS = index.EMPTY_ARRAY;
...
class ParameterVariable extends LocalVariable {
    ...
    markReassigned() {
        ...
        this.expressionsUseTheKnownValue = index.EMPTY_ARRAY;
    }
    deoptimizeCache() {
        this.markReassigned();
    }
...
class Scope {
    ...
    addDeclaration(identifier, context, init, kind) {
        ...
        if (existingVariable) {
            ...
            context.error(index.logRedeclarationError(name), identifier.start);
        }
        ...
        if (kind === 'var') {
            ...
            if (existingVariable) {
                ...
                if (existingKind === 'parameter' &&
                    ...
                    existingVariable.declarations[0].parent.type === index.CatchClause) {
                ...
                return context.error(index.logRedeclarationError(name), identifier.start);
            }
...
class ParameterScope extends ChildScope {
    ...
    addParameterDeclaration(identifier) {
        ...
        if (existingParameter) {
            return this.context.error(index.logDuplicateArgumentNameError(name), start);
        }
...
class ExpressionStatement extends NodeBase {
    initialise() {
        ...
        if (this.directive &&
            ...
            this.parent.type === index.Program) {
            this.scope.context.log(index.LOGLEVEL_WARN, 
            // This is necessary, because either way (deleting or not) can lead to errors.
            index.logModuleLevelDirective(this.directive, this.scope.context.module.id), this.start);
        }
    }
    removeAnnotations(code) {
    ...
    shouldBeIncluded(context) {
        if (this.directive && this.directive !== 'use strict')
            return this.parent.type !== index.Program;
...
class BlockStatement extends NodeBase {
    ...
    addImplicitReturnExpressionToScope() {
        ...
        if (!lastStatement || lastStatement.type !== index.ReturnStatement) {
...
class FunctionBase extends NodeBase {
    ...
    onlyFunctionCallUsed() {
        let variable = null;
        if (this.parent.type === index.VariableDeclarator) {
            variable = this.parent.id.variable ?? null;
        }
        if (this.parent.type === index.ExportDefaultDeclaration) {
...
class ArrowFunctionExpression extends FunctionBase {
    ...
    onlyFunctionCallUsed() {
        const isIIFE = this.parent.type === index.CallExpression &&
...
class ObjectPattern extends NodeBase {
    addExportedVariables(variables, exportNamesByVariable) {
        for (const property of this.properties) {
            if (property.type === index.Property) {
    ...
    initialise() {
        ...
        if (this.left instanceof Identifier) {
            ...
            if (variable?.kind === 'const') {
                this.scope.context.error(index.logConstVariableReassignError(), this.left.start);
            }
        }
        this.left.setAssignedValue(this.right);
    }
    render(code, options, { preventASI, renderedParentType, renderedSurroundingElement } = index.BLANK) {
            ...
            if (left instanceof Identifier) {
                ...
                if (exportNames) {
                    if (exportNames.length === 1) {
                        ...
                        renderSystemExportSequenceAfterExpression(variable, start, end, parent.type !== index.ExpressionStatement, code, options);
                    }
                    return;
                }
            }
                ...
                if (systemPatternExports.length > 0) {
                    renderSystemExportFunction(systemPatternExports, start, end, renderedSurroundingElement === index.ExpressionStatement, code, options);
                    return;
                }
            }
        }
        if (left.included &&
            ...
            (renderedSurroundingElement === index.ExpressionStatement ||
                renderedSurroundingElement === index.ArrowFunctionExpression)) {
...
class AssignmentPattern extends NodeBase {
    ...
    render(code, options, { isShorthandProperty } = index.BLANK) {
...
class BinaryExpression extends NodeBase {
    ...
    render(code, options, { renderedSurroundingElement } = index.BLANK) {
...
class MemberExpression extends NodeBase {
    ...
    render(code, options, { renderedParentType, isCalleeOfRenderedParent, renderedSurroundingElement } = index.BLANK) {
        if (this.variable || this.isUndefined) {
    ...
    disallowNamespaceReassignment() {
        if (this.object instanceof Identifier) {
            ...
            if (variable.isNamespace) {
                ...
                this.scope.context.log(index.LOGLEVEL_WARN, index.logIllegalImportReassignment(this.object.name, this.scope.context.module.id), this.start);
            }
        }
    }
    getPropertyKey() {
...
function resolveNamespaceVariables(baseVariable, path, astContext) {
    ...
    if (!variable) {
        if (path.length === 1) {
            ...
            astContext.log(index.LOGLEVEL_WARN, index.logMissingExport(exportName, astContext.module.id, fileName), path[0].pos);
            return 'undefined';
        }
        return null;
    }
...
class CallExpressionBase extends NodeBase {
    ...
    deoptimizeCache() {
        if (this.returnExpression?.[0] !== UNKNOWN_EXPRESSION) {
            ...
            this.expressionsToBeDeoptimized = index.EMPTY_SET;
            this.deoptimizableDependentExpressions = index.EMPTY_ARRAY;
...
class CallExpression extends CallExpressionBase {
    ...
    bind() {
        ...
        if (this.callee instanceof Identifier) {
            ...
            if (variable.isNamespace) {
                this.scope.context.log(index.LOGLEVEL_WARN, index.logCannotCallNamespace(this.callee.name), this.start);
            }
            if (this.callee.name === 'eval') {
                this.scope.context.log(index.LOGLEVEL_WARN, index.logEval(this.scope.context.module.id), this.start);
            }
        }
        this.interaction = {
            args: [
...
function isStaticBlock(statement) {
    return statement.type === index.StaticBlock;
}
...
class ClassExpression extends ClassNode {
    render(code, options, { renderedSurroundingElement } = index.BLANK) {
        super.render(code, options);
        if (renderedSurroundingElement === index.ExpressionStatement) {
...
class ConditionalExpression extends NodeBase {
    ...
    render(code, options, { isCalleeOfRenderedParent, preventASI, renderedParentType, renderedSurroundingElement } = index.BLANK) {
        const usedBranch = this.getUsedBranch();
...
class IfStatement extends NodeBase {
    ...
    renderHoistedDeclarations(hoistedDeclarations, code, getPropertyAccess) {
        ...
        if (hoistedVariables) {
            ...
            const needsBraces = parentType !== index.Program && parentType !== index.BlockStatement;
...
class VariableDeclarator extends NodeBase {
    ...
    render(code, options) {
        ...
        if (init) {
            ...
            init.render(code, options, renderId ? index.BLANK : { renderedSurroundingElement: index.ExpressionStatement });
        }
...
class MetaProperty extends NodeBase {
    ...
    render(code, renderOptions) {
        ...
        if (referenceId) {
            ...
            const relativePath = index.normalize(path$n.relative(path$n.dirname(chunkId), fileName));
...
class PanicError extends NodeBase {
    initialise() {
        ...
        const parseError = index.getRollupError(index.logParseError(this.message));
        const moduleParseError = index.logModuleParseError(parseError, id);
        return index.error(moduleParseError);
    }
}
...
class ParseError extends NodeBase {
    initialise() {
        ...
        // This simulates the current nested error structure. We could also just
        // replace it with a flat error.
        const parseError = index.getRollupError(index.logParseError(this.message, pos));
        const moduleParseError = index.logModuleParseError(parseError, id);
        ...
        for (const node of this.body) {
            if (node.hasEffects(context)) {
                if (this.scope.context.options.experimentalLogSideEffects && !this.hasLoggedEffect) {
                    ...
                    log(index.LOGLEVEL_INFO, index.logFirstSideEffect(code, module.id, index.locate(code, node.start, { offsetLine: 1 })), node.start);
                }
        ...
        if (this.invalidAnnotations)
            for (const { start, end, type } of this.invalidAnnotations) {
                ...
                if (type === 'pure' || type === 'noSideEffects') {
                    this.scope.context.log(index.LOGLEVEL_WARN, index.logInvalidAnnotation(this.scope.context.code.slice(start, end), this.scope.context.module.id, type), start);
                }
            }
    }
    render(code, options) {
...
class SequenceExpression extends NodeBase {
    ...
    render(code, options, { renderedParentType, isCalleeOfRenderedParent, preventASI } = index.BLANK) {
        let includedNodes = 0;
...
class TaggedTemplateExpression extends CallExpressionBase {
    bind() {
        super.bind();
        if (this.tag.type === index.Identifier) {
            const name = this.tag.name;
            const variable = this.scope.findVariable(name);
            if (variable.isNamespace) {
                this.scope.context.log(index.LOGLEVEL_WARN, index.logCannotCallNamespace(name), this.start);
            }
        }
    }
    hasEffects(context) {
...
class ModuleScope extends ChildScope {
    ...
    addDeclaration(identifier, context, init, kind) {
        if (this.context.module.importDescriptions.has(identifier.name)) {
            context.error(index.logRedeclarationError(identifier.name), identifier.start);
        }
        return super.addDeclaration(identifier, context, init, kind);
    }
...
class ThisExpression extends NodeBase {
    ...
    initialise() {
        ...
        if (this.alias === 'undefined') {
            this.scope.context.log(index.LOGLEVEL_WARN, index.logThisIsUndefined(), this.start);
        }
    }
    render(code) {
        if (this.alias !== null) {
            ...
            if (exportNames) {
                if (this.prefix) {
                    if (exportNames.length === 1) {
                        ...
                        renderSystemExportSequenceAfterExpression(variable, this.start, this.end, this.parent.type !== index.ExpressionStatement, code, options);
                    }
                }
                else {
                    const operator = this.operator[0];
                    renderSystemExportSequenceBeforeExpression(variable, this.start, this.end, this.parent.type !== index.ExpressionStatement, code, options, `${_}${operator}${_}1`);
                }
            }
        }
    }
...
function areAllDeclarationsIncludedAndNotExported(declarations, exportNamesByVariable) {
    for (const declarator of declarations) {
        if (!declarator.id.included)
            return false;
        if (declarator.id.type === index.Identifier) {
...
class VariableDeclaration extends NodeBase {
    ...
    include(context, includeChildrenRecursively, { asSingleStatement } = index.BLANK) {
        this.included = true;
        for (const declarator of this.declarations) {
    ...
    render(code, options, nodeRenderOptions = index.BLANK) {
...
function convertProgram(buffer, parent, parentScope) {
    return convertNode(parent, parentScope, 0, index.getAstBuffer(buffer));
}
...
const bufferParsers = [
    ...
    function arrowFunctionExpression(node, position, buffer) {
        ...
        const annotations = (node.annotations = index.convertAnnotations(buffer[position + 1], buffer));
    ...
    function assignmentExpression(node, position, buffer) {
        const { scope } = node;
        node.operator = index.FIXED_STRINGS[buffer[position]];
    ...
    function callExpression(node, position, buffer) {
        ...
        node.annotations = index.convertAnnotations(buffer[position + 1], buffer);
    ...
    function importExpression(node, position, buffer) {
        ...
        node.sourceAstNode = index.convertNode(buffer[position], buffer);
    ...
    function methodDefinition(node, position, buffer) {
        ...
        node.kind = index.FIXED_STRINGS[buffer[position + 4]];
    },
    function newExpression(node, position, buffer) {
        const { scope } = node;
        node.annotations = index.convertAnnotations(buffer[position], buffer);
    ...
    function program(node, position, buffer) {
        const { scope } = node;
        node.body = convertNodeList(node, scope, buffer[position], buffer);
        node.invalidAnnotations = index.convertAnnotations(buffer[position + 1], buffer);
    },
    function property(node, position, buffer) {
        ...
        node.kind = index.FIXED_STRINGS[buffer[position + 3]];
...
function getAttributesFromImportExpression(node) {
    ...
    if (!(options instanceof ObjectExpression)) {
        if (options) {
            context.module.log(index.LOGLEVEL_WARN, index.logImportAttributeIsInvalid(context.module.id), start);
        }
        return index.EMPTY_OBJECT;
    }
    ...
    if (!(assertProperty instanceof ObjectExpression)) {
        context.module.log(index.LOGLEVEL_WARN, index.logImportOptionsAreInvalid(context.module.id), start);
        return index.EMPTY_OBJECT;
    }
    const assertFields = assertProperty.properties
        ...
        context.module.log(index.LOGLEVEL_WARN, index.logImportAttributeIsInvalid(context.module.id), property.start);
        return null;
    })
...
function getVariableForExportNameRecursive(target, name, importerForSideEffects, isExportAllSearch, searchedNamesAndModules = new Map()) {
    ...
    if (searchedModules) {
        if (searchedModules.has(target)) {
            return isExportAllSearch ? [null] : index.error(index.logCircularReexport(name, target.id));
        }
...
class Module {
    ...
    basename() {
        const base = path$n.basename(this.id);
        const extension = path$n.extname(this.id);
    ...
    getSyntheticNamespace() {
        ...
        if (!this.syntheticNamespace) {
            return index.error(index.logSyntheticNamedExportsNeedNamespaceExport(this.id, this.info.syntheticNamedExports));
        }
        return this.syntheticNamespace;
    }
    getVariableForExportName(name, { importerForSideEffects, isExportAllSearch, onlyExplicit, searchedNamesAndModules } = index.EMPTY_OBJECT) {
        if (name[0] === '*') {
            if (name.length === 1) {
                // export * from './other'
        ...
        if (reexportDeclaration) {
            ...
            if (!variable) {
                return this.error(index.logMissingExport(reexportDeclaration.localName, this.id, reexportDeclaration.module.id), reexportDeclaration.start);
            }
    ...
    includeAllExports(includeNamespaceMembers) {
        ...
        for (const exportName of this.exports.keys()) {
            if (includeNamespaceMembers || exportName !== this.info.syntheticNamedExports) {
                ...
                if (!variable) {
                    return index.error(index.logMissingEntryExport(exportName, this.id));
                }
    ...
    render(options) {
        ...
        if (usesTopLevelAwait && options.format !== 'es' && options.format !== 'system') {
            return index.error(index.logInvalidFormatForTopLevelAwait(this.id, options.format));
        }
    ...
    async setSource({ ast, code, customTransformCache, originalCode, originalSourcemap, resolvedIds, sourcemapChain, transformDependencies, transformFiles, ...moduleOptions }) {
        ...
        if (ast) {
            ...
            const astBuffer = await index.parseAsync_1(code, false);
            timeStart('generate ast', 3);
    ...
    traceVariable(name, { importerForSideEffects, isExportAllSearch, searchedNamesAndModules } = index.EMPTY_OBJECT) {
        ...
        if (importDescription) {
            ...
            if (!declaration) {
                return this.error(index.logMissingExport(importDescription.name, this.id, otherModule.id), importDescription.start);
            }
            return declaration;
        }
        return null;
    ...
    assertUniqueExportName(name, nodeStart) {
        if (this.exports.has(name) || this.reexportDescriptions.has(name)) {
            this.error(index.logDuplicateExportError(name), nodeStart);
        }
    }
    addExport(node) {
    ...
    addImport(node) {
        ...
        for (const specifier of node.specifiers) {
            ...
            if (this.scope.variables.has(localName) || this.importDescriptions.has(localName)) {
                this.error(index.logRedeclarationError(localName), specifier.local.start);
            }
    ...
    addLocationToLogProps(properties, pos) {
        ...
        const location = index.locate(code, pos, { offsetLine: 1 });
        if (location) {
            ...
            try {
                ...
                this.options.onLog(index.LOGLEVEL_WARN, index.logInvalidSourcemapForError(error_, this.id, column, line, pos));
            }
            index.augmentCodeLocation(properties, { column, line }, code, this.id);
        }
    }
    ...
    addSource(source, declaration) {
        ...
        if (existingAttributes) {
            if (doAttributesDiffer(existingAttributes, parsedAttributes)) {
                this.log(index.LOGLEVEL_WARN, index.logInconsistentImportAttributes(existingAttributes, parsedAttributes, source, this.id), declaration.start);
            }
        }
        else {
    ...
    getVariableFromNamespaceReexports(name, importerForSideEffects, searchedNamesAndModules) {
        ...
        if (foundInternalDeclarations.size > 0) {
            ...
            this.options.onLog(index.LOGLEVEL_WARN, index.logNamespaceConflict(name, this.id, foundDeclarationList.map(([, module]) => module.id)));
        ...
        if (foundExternalDeclarations.size > 0) {
            ...
            if (foundDeclarationList.length > 1) {
                this.options.onLog(index.LOGLEVEL_WARN, index.logAmbiguousExternalNamespaces(name, this.id, usedDeclaration.module.id, foundDeclarationList.map(declaration => declaration.module.id)));
            }
    ...
    shimMissingExport(name) {
        this.options.onLog(index.LOGLEVEL_WARN, index.logShimmedExport(this.id, name));
        this.exports.set(name, MISSING_EXPORT_SHIM_DESCRIPTION);
    }
    tryParse() {
        try {
            return index.parseAst(this.info.code);
        }
        catch (error_) {
            return this.error(index.logModuleParseError(error_, this.id), error_.pos);
        }
    }
}
...
function warnOnBuiltins(log, dependencies) {
    ...
    if (externalBuiltins.length === 0)
        return;
    log(index.LOGLEVEL_WARN, index.logMissingNodeBuiltins(externalBuiltins));
}
...
function iife(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, indent: t, intro, namedExportsMode, log, outro, snippets }, { compact, esModule, extend, freeze, externalLiveBindings, reexportProtoFromExternal, globals, interop, name, generatedCode: { symbols }, strict }) {
    ...
    if (name && useVariableAssignment && !isLegal(name)) {
        return index.error(index.logIllegalIdentifierAsName(name));
    }
    warnOnBuiltins(log, dependencies);
    ...
    if (hasExports && !name) {
        log(index.LOGLEVEL_WARN, index.logMissingNameOptionForIifeExport());
    }
    if (namedExportsMode && hasExports) {
        if (extend) {
...
function umd(magicString, { accessedGlobals, dependencies, exports, hasDefaultExport, hasExports, id, indent: t, intro, namedExportsMode, log, outro, snippets }, { amd, compact, esModule, extend, externalLiveBindings, freeze, interop, name, generatedCode: { symbols }, globals, noConflict, reexportProtoFromExternal, strict }) {
    ...
    if (hasExports && !name) {
        return index.error(index.logMissingNameOptionForUmdExport());
    }
    warnOnBuiltins(log, dependencies);
...
async function createAddons(options, outputPluginDriver, chunk) {
    try {
        ...
        return index.error(index.logAddonNotGenerated(error_.message, error_.hook, error_.plugin));
    }
}
...
function getExportMode(chunk, { exports: exportMode, name, format }, facadeModuleId, log) {
    ...
    if (exportMode === 'default') {
        if (exportKeys.length !== 1 || exportKeys[0] !== 'default') {
            return index.error(index.logIncompatibleExportOptionValue('default', exportKeys, facadeModuleId));
        }
    }
    else if (exportMode === 'none' && exportKeys.length > 0) {
        return index.error(index.logIncompatibleExportOptionValue('none', exportKeys, facadeModuleId));
    }
    if (exportMode === 'auto') {
        if (exportKeys.length === 0) {
            ...
            if (format !== 'es' && format !== 'system' && exportKeys.includes('default')) {
                log(index.LOGLEVEL_WARN, index.logMixedExport(facadeModuleId, name));
            }
            exportMode = 'named';
        }
    }
...
const getHashPlaceholderGenerator = () => {
    ...
    return (optionName, hashSize) => {
        if (hashSize > MAX_HASH_SIZE) {
            return index.error(index.logFailedValidation(`Hashes cannot be longer than ${MAX_HASH_SIZE} characters, received ${hashSize}. Check the "${optionName}" option.`));
        }
        ...
        if (placeholder.length > hashSize) {
            return index.error(index.logFailedValidation(`To generate hashes for this number of chunks (currently ${nextIndex}), you need a minimum hash size of ${placeholder.length}, received ${hashSize}. Check the "${optionName}" option.`));
        }
        return placeholder;
    };
};
...
function renderNamePattern(pattern, patternName, replacements) {
    if (index.isPathFragment(pattern))
        return index.error(index.logFailedValidation(`Invalid pattern "${pattern}" for "${patternName}", patterns can be neither absolute nor relative paths. If you want your files to be stored in a subdirectory, write its name without a leading slash like this: subdirectory/pattern.`));
    return pattern.replace(/\[(\w+)(:\d+)?]/g, (_match, type, size) => {
        if (!replacements.hasOwnProperty(type) || (size && type !== 'hash')) {
            return index.error(index.logFailedValidation(`"[${type}${size || ''}]" is not a valid placeholder in the "${patternName}" pattern.`));
        }
        const replacement = replacements[type](size && Number.parseInt(size.slice(1)));
        if (index.isPathFragment(replacement))
            return index.error(index.logFailedValidation(`Invalid substitution "${replacement}" for placeholder "[${type}]" in "${patternName}" pattern, can be neither absolute nor relative path.`));
        return replacement;
    });
}
function makeUnique(name, { [lowercaseBundleKeys]: reservedLowercaseBundleKeys }) {
    if (!reservedLowercaseBundleKeys.has(name.toLowerCase()))
        return name;
    const extension = path$n.extname(name);
...
function getGlobalName(chunk, globals, hasExports, log) {
    ...
    if (hasExports) {
        log(index.LOGLEVEL_WARN, index.logMissingGlobalName(chunk.id, chunk.variableName));
...
class Chunk {
    ...
    getImportPath(importer) {
        return escapeId(index.getImportPath(importer, this.getFileName(), this.outputOptions.format === 'amd' && !this.outputOptions.amd.forceJsExtensionForImports, true));
    }
            ...
            do {
                ...
                if (alternativeReexportModule) {
                    ...
                    if (exportingChunk !== exportChunk) {
                        this.inputOptions.onLog(index.LOGLEVEL_WARN, index.logCyclicCrossChunkReexport(
        ...
        for (const variable of this.imports) {
            ...
            if (module instanceof ExternalModule) {
                ...
                if (imported !== 'default' && imported !== '*' && interop(module.id) === 'defaultOnly') {
                    return index.error(index.logUnexpectedNamedImport(module.id, imported, false));
                }
            }
            else {
    ...
    getPreserveModulesChunkNameFromModule(module) {
        ...
        const sanitizedId = sanitizeFileName(index.normalize(module.id.split(QUERY_HASH_REGEX, 1)[0]));
        const extensionName = path$n.extname(sanitizedId);
        const idWithoutExtension = NON_ASSET_EXTENSIONS.has(extensionName)
            ? sanitizedId.slice(0, -extensionName.length)
            : sanitizedId;
        if (index.isAbsolute(idWithoutExtension)) {
            return preserveModulesRoot && path$n.resolve(idWithoutExtension).startsWith(preserveModulesRoot)
                ? idWithoutExtension.slice(preserveModulesRoot.length).replace(/^[/\\]/, '')
                : index.relative(this.inputBase, idWithoutExtension);
        }
        else {
            return (this.outputOptions.virtualDirname.replace(/\/$/, '') + '/' + path$n.basename(idWithoutExtension));
        }
    }
        ...
        for (let exportName of this.getExportNames()) {
            ...
            if (exportName[0] === '*') {
                ...
                if (interop(id) === 'defaultOnly') {
                    this.inputOptions.onLog(index.LOGLEVEL_WARN, index.logUnexpectedNamespaceReexport(id));
                }
                ...
                if (module instanceof Module) {
                    ...
                    imported = variable.name;
                    if (imported !== 'default' && imported !== '*' && interop(module.id) === 'defaultOnly') {
                        return index.error(index.logUnexpectedNamedImport(module.id, imported, true));
                    }
                    needsLiveBinding =
    ...
    renderModules(fileName) {
        ...
        if (isEmpty && this.getExportNames().length === 0 && dependencies.size === 0) {
            onLog(index.LOGLEVEL_WARN, index.logEmptyChunk(this.getChunkName()));
        }
...
function getChunkNameFromModule(module) {
    return getPredefinedChunkNameFromModule(module) ?? index.getAliasName(module.id);
}
...
function getOptimizedChunks(chunks, minChunkSize, sideEffectAtoms, sizeByAtom, log) {
    ...
    if (minChunkSize > 1) {
        log('info', index.logOptimizeChunkStatus(chunks.length, chunkPartition.small.size, 'Initially'));
    }
    mergeChunks(chunkPartition, minChunkSize, sideEffectAtoms, sizeByAtom);
    if (minChunkSize > 1) {
        log('info', index.logOptimizeChunkStatus(chunkPartition.small.size + chunkPartition.big.size, chunkPartition.small.size, 'After merging chunks'));
    }
    timeEnd('optimize chunks', 3);
            ...
            for (const segment of line) {
                ...
                if (traced) {
                    ...
                    if (sourceIndex === undefined) {
                        ...
                        return index.error(index.logConflictingSourcemapSources(filename));
                    }
...
function getLinkMap(log) {
    return function linkMap(source, map) {
        ...
        log(index.LOGLEVEL_WARN, index.logSourcemapBroken(map.plugin));
        return new Link({
            mappings: [],
            names: []
        }, [source]);
...
function getCollapsedSourcemap(id, originalCode, originalSourcemap, sourcemapChain, linkMap) {
    ...
    if (originalSourcemap) {
        ...
        const directory = path$n.dirname(id) || '.';
        const sourceRoot = originalSourcemap.sourceRoot || '.';
        const baseSources = sources.map((source, index) => new Source(path$n.resolve(directory, sourceRoot, source), sourcesContent[index]));
...
function collapseSourcemaps(file, map, modules, bundleSourcemapChain, excludeContent, log) {
    ...
    if (file) {
        const directory = path$n.dirname(file);
        sources = sources.map((source) => path$n.relative(directory, source));
        file = path$n.basename(file);
    }
...
let textEncoder;
const getHash64 = input => index.xxhashBase64Url_1(ensureBuffer(input));
const getHash36 = input => index.xxhashBase36_1(ensureBuffer(input));
const getHash16 = input => index.xxhashBase16_1(ensureBuffer(input));
const hasherByType = {
    base36: getHash36,
    base64: getHash64,
    hex: getHash16
...
async function transformChunk(magicString, fileName, usedModules, chunkGraph, options, outputPluginDriver, log) {
    ...
    if (sourcemap) {
        ...
        let resultingFile;
        if (file)
            resultingFile = path$n.resolve(sourcemapFile || file);
        else if (dir)
            resultingFile = path$n.resolve(dir, fileName);
        else
            resultingFile = path$n.resolve(fileName);
        ...
        for (let sourcesIndex = 0; sourcesIndex < map.sources.length; ++sourcesIndex) {
            ...
            if (typeof ignoreList !== 'boolean') {
                index.error(index.logFailedValidation('sourcemapIgnoreList function must return a boolean.'));
            }
            if (ignoreList) {
            ...
            if (sourcemapPathTransform) {
                ...
                if (typeof sourcePath !== 'string') {
                    index.error(index.logFailedValidation(`sourcemapPathTransform function must return a string.`));
                }
            }
            map.sources[sourcesIndex] = index.normalize(sourcePath);
        }
...
function emitSourceMapAndGetComment(fileName, map, pluginDriver, { sourcemap, sourcemapBaseUrl }) {
    ...
    if (sourcemap === 'inline') {
        ...
        const sourcemapFileName = path$n.basename(fileName);
    ...
    finaliseAssets(bundle) {
        if (this.outputOptions.validate) {
            for (const file of Object.values(bundle)) {
                if ('code' in file) {
                    try {
                        index.parseAst(file.code);
                    }
                    catch (error_) {
                        this.inputOptions.onLog(index.LOGLEVEL_WARN, index.logChunkInvalid(file, error_));
                    }
                }
            }
        }
...
function validateOptionsForMultiChunkOutput(outputOptions, log) {
    if (outputOptions.format === 'umd' || outputOptions.format === 'iife')
        return index.error(index.logInvalidOption('output.format', index.URL_OUTPUT_FORMAT, 'UMD and IIFE output formats are not supported for code-splitting builds', outputOptions.format));
    if (typeof outputOptions.file === 'string')
        return index.error(index.logInvalidOption('output.file', index.URL_OUTPUT_DIR, 'when building multiple chunks, the "output.dir" option must be used, not "output.file". To inline dynamic imports, set the "inlineDynamicImports" option'));
    if (outputOptions.sourcemapFile)
        return index.error(index.logInvalidOption('output.sourcemapFile', index.URL_OUTPUT_SOURCEMAPFILE, '"output.sourcemapFile" is only supported for single-file builds'));
    if (!outputOptions.amd.autoId && outputOptions.amd.id)
        log(index.LOGLEVEL_WARN, index.logInvalidOption('output.amd.id', index.URL_OUTPUT_AMD_ID, 'this option is only properly supported for single-file builds. Use "output.amd.autoId" and "output.amd.basePath" instead'));
}
...
function getAbsoluteEntryModulePaths(includedModules, preserveModules) {
    ...
    for (const module of includedModules) {
        if ((module.info.isEntry || preserveModules) && index.isAbsolute(module.id)) {
...
function addModuleToManualChunk(alias, module, manualChunkAliasByEntry) {
    ...
    if (typeof existingAlias === 'string' && existingAlias !== alias) {
        return index.error(index.logCannotAssignModuleToChunk(module.id, alias, existingAlias));
    }
...
function resolveIdViaPlugins(source, importer, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes) {
    ...
    if (skip) {
        ...
        replaceContext = (pluginContext, plugin) => ({
            ...pluginContext,
            resolve: (source, importer, { attributes, custom, isEntry, skipSelf } = index.BLANK) => {
                skipSelf ??= true;
                return moduleLoaderResolveId(source, importer, custom, isEntry, attributes || index.EMPTY_OBJECT, skipSelf ? [...skip, { importer, plugin, source }] : skip);
            }
        });
    }
...
async function resolveId(source, importer, preserveSymlinks, pluginDriver, moduleLoaderResolveId, skip, customOptions, isEntry, attributes) {
    ...
    return addJsExtensionIfNecessary(importer ? path$n.resolve(path$n.dirname(importer), source) : path$n.resolve(source), preserveSymlinks);
}
...
async function findFile(file, preserveSymlinks) {
    try {
        ...
        if (!preserveSymlinks && stats.isSymbolicLink())
            return await findFile(await fsp.realpath(file), preserveSymlinks);
        if ((preserveSymlinks && stats.isSymbolicLink()) || stats.isFile()) {
            // check case
            const name = path$n.basename(file);
            const files = await fsp.readdir(path$n.dirname(file));
...
function uncacheablePluginError(pluginName) {
    if (pluginName.startsWith(ANONYMOUS_PLUGIN_PREFIX) ||
        ...
        return index.error(index.logAnonymousPluginCache());
    }
    return index.error(index.logDuplicatePluginName(pluginName));
}
...
const getOnLog = (config, logLevel, printLog = defaultPrintLog) => {
    ...
    if (onLog) {
        const minimalPriority = index.logLevelPriority[logLevel];
        return (level, log) => onLog(level, addLogToString(log), (level, handledLog) => {
            if (level === index.LOGLEVEL_ERROR) {
                return index.error(normalizeLog(handledLog));
            }
            if (index.logLevelPriority[level] >= minimalPriority) {
...
const getDefaultOnLog = (printLog, onwarn) => onwarn
    ? (level, log) => {
        if (level === index.LOGLEVEL_WARN) {
            onwarn(addLogToString(log), warning => printLog(index.LOGLEVEL_WARN, normalizeLog(warning)));
        }
        else {
            printLog(level, log);
        }
...
const defaultPrintLog = (level, { message }) => {
    switch (level) {
        case index.LOGLEVEL_WARN: {
            return console.warn(message);
        }
        case index.LOGLEVEL_DEBUG: {
...
function warnUnknownOptions(passedOptions, validOptions, optionType, log, ignoredKeys = /$./) {
    ...
    if (unknownOptions.length > 0) {
        log(index.LOGLEVEL_WARN, index.logUnknownOption(optionType, unknownOptions, [...validOptionSet].sort()));
    }
}
const treeshakePresets = {
    recommended: {
        ...
        manualPureFunctions: index.EMPTY_ARRAY,
...
const objectifyOptionWithPresets = (presets, optionName, urlSnippet, additionalValues) => (value) => {
    if (typeof value === 'string') {
        ...
        index.error(index.logInvalidOption(optionName, urlSnippet, `valid values are ${additionalValues}${index.printQuotedStringList(Object.keys(presets))}. You can also supply an object for more fine-grained control`, value));
    }
...
const getOptionWithPreset = (value, presets, optionName, urlSnippet, additionalValues) => {
    ...
    if (presetName) {
        ...
        if (preset) {
            ...
            index.error(index.logInvalidOption(`${optionName}.preset`, urlSnippet, `valid values are ${index.printQuotedStringList(Object.keys(presets))}`, presetName));
        }
    }
...
async function transform(source, module, pluginDriver, log) {
        ...
        if (typeof result === 'string') {
            ...
            if (result.code == null) {
                if (result.map || result.ast) {
                    log(index.LOGLEVEL_WARN, index.logNoTransformMapOrAstWithoutCode(plugin.name));
                }
                return previousCode;
            }
            ({ code, map, ast } = result);
    ...
    const getLogHandler = (handler) => (log, pos) => {
        ...
        if (pos)
            index.augmentCodeLocation(log, pos, currentSource, id);
        log.id = id;
        log.hook = 'transform';
        handler(log);
    };
    ...
    try {
        code = await pluginDriver.hookReduceArg0('transform', [currentSource, id], transformReducer, (pluginContext, plugin) => {
            ...
            return {
                ...
                error(error_, pos) {
                    ...
                    if (pos)
                        index.augmentCodeLocation(error_, pos, currentSource, id);
                    error_.id = id;
                ...
                setAssetSource() {
                    return this.error(index.logInvalidSetAssetSourceCall());
                },
        ...
        return index.error(index.logPluginError(error_, pluginName, { hook: 'transform', id }));
    }
...
class ModuleLoader {
    ...
    async preloadModule(resolvedId) {
        const module = await this.fetchModule(this.getResolvedIdWithDefaults(resolvedId, index.EMPTY_OBJECT), undefined, false, resolvedId.resolveDependencies ? RESOLVE_DEPENDENCIES : true);
        return module.info;
    }
    ...
    async addModuleSource(id, importer, module) {
        ...
        try {
            source = await this.graph.fileOperationQueue.run(async () => {
                ...
                return await fsp.readFile(id, 'utf8');
            });
        }
        catch (error_) {
            let message = `Could not load ${id}`;
            if (importer)
                message += ` (imported by ${index.relativeId(importer)})`;
        ...
        const sourceDescription = typeof source === 'string'
            ...
            : source != null && typeof source === 'object' && typeof source.code === 'string'
                ? source
                : index.error(index.logBadLoader(id));
    ...
    async fetchDynamicDependencies(module, resolveDynamicImportPromises) {
        const dependencies = await Promise.all(resolveDynamicImportPromises.map(resolveDynamicImportPromise => resolveDynamicImportPromise.then(async ([dynamicImport, resolvedId]) => {
            ...
            return (dynamicImport.resolution = await this.fetchResolvedDependency(index.relativeId(resolvedId.id), module.id, resolvedId));
        })));
        for (const dependency of dependencies) {
            if (dependency) {
    ...
    async fetchModule({ attributes, id, meta, moduleSideEffects, syntheticNamedExports }, importer, isEntry, isPreload) {
        ...
        if (existingModule instanceof Module) {
            if (importer && doAttributesDiffer(attributes, existingModule.info.attributes)) {
                this.options.onLog(index.LOGLEVEL_WARN, index.logInconsistentImportAttributes(existingModule.info.attributes, attributes, id, importer));
            }
            await this.handleExistingModule(existingModule, isEntry, isPreload);
            return existingModule;
        }
        if (existingModule instanceof ExternalModule) {
            return index.error(index.logExternalModulesCannotBeTransformedToModules(existingModule.id));
        }
    ...
    fetchResolvedDependency(source, importer, resolvedId) {
        if (resolvedId.external) {
            ...
            if (!externalModule) {
                externalModule = new ExternalModule(this.options, id, moduleSideEffects, meta, external !== 'absolute' && index.isAbsolute(id), attributes);
                this.modulesById.set(id, externalModule);
            }
            else if (!(externalModule instanceof ExternalModule)) {
                return index.error(index.logInternalIdCannotBeExternal(source, importer));
            }
            else if (doAttributesDiffer(externalModule.info.attributes, attributes)) {
                this.options.onLog(index.LOGLEVEL_WARN, index.logInconsistentImportAttributes(externalModule.info.attributes, attributes, source, importer));
            }
        ...
        if (resolveIdResult) {
            if (typeof resolveIdResult === 'object') {
                ...
                return {
                    ...
                    external: external &&
                        (external === 'relative' ||
                            !index.isAbsolute(resolveIdResult.id) ||
                            (external === true &&
    ...
    getResolveStaticDependencyPromises(module) {
        ...
        return Array.from(module.sourcesWithAttributes, async ([source, attributes]) => [
            ...
            (module.resolvedIds[source] =
                module.resolvedIds[source] ||
                    this.handleInvalidResolvedId(await this.resolveId(source, module.id, index.EMPTY_OBJECT, false, attributes), source, module.id, attributes))
        ]);
    }
    ...
    handleInvalidResolvedId(resolvedId, source, importer, attributes) {
        if (resolvedId === null) {
            if (index.isRelative(source)) {
                return index.error(index.logUnresolvedImport(source, importer));
            }
            this.options.onLog(index.LOGLEVEL_WARN, index.logUnresolvedImportTreatedAsExternal(source, importer));
            return {
                attributes,
                external: true,
                id: source,
            ...
            this.options.onLog(index.LOGLEVEL_WARN, index.logExternalSyntheticExports(source, importer));
        }
        return resolvedId;
    }
    async loadEntryModule(unresolvedId, isEntry, importer, implicitlyLoadedBefore, isLoadForManualChunks = false) {
        const resolveIdResult = await resolveId(unresolvedId, importer, this.options.preserveSymlinks, this.pluginDriver, this.resolveId, null, index.EMPTY_OBJECT, true, index.EMPTY_OBJECT);
        if (resolveIdResult == null) {
            return index.error(implicitlyLoadedBefore === null
                ? index.logUnresolvedEntry(unresolvedId)
                : index.logUnresolvedImplicitDependant(unresolvedId, implicitlyLoadedBefore));
        }
        ...
        if (resolveIdResult === false || isExternalModules) {
            return index.error(implicitlyLoadedBefore === null
                ? isExternalModules && isLoadForManualChunks
                    ? index.logExternalModulesCannotBeIncludedInManualChunks(unresolvedId)
                    : index.logEntryCannotBeExternal(unresolvedId)
                : index.logImplicitDependantCannotBeExternal(unresolvedId, implicitlyLoadedBefore));
        }
        return this.fetchModule(this.getResolvedIdWithDefaults(typeof resolveIdResult === 'object'
            ? resolveIdResult
            : { id: resolveIdResult }, index.EMPTY_OBJECT), undefined, isEntry, false);
    }
        ...
        if (resolution == null) {
            ...
            if (existingResolution) {
                if (doAttributesDiffer(existingResolution.attributes, attributes)) {
                    this.options.onLog(index.LOGLEVEL_WARN, index.logInconsistentImportAttributes(existingResolution.attributes, attributes, specifier, importer));
                }
                return existingResolution;
            }
            return (module.resolvedIds[specifier] = this.handleInvalidResolvedId(await this.resolveId(specifier, module.id, index.EMPTY_OBJECT, false, attributes), specifier, module.id, attributes));
        }
...
function normalizeRelativeExternalId(source, importer) {
    return index.isRelative(source)
        ? importer
            ? path$n.resolve(importer, '..', source)
            : path$n.resolve(source)
        : source;
}
...
function isNotAbsoluteExternal(id, source, makeAbsoluteExternalsRelative) {
    return (makeAbsoluteExternalsRelative === true ||
        (makeAbsoluteExternalsRelative === 'ifRelativeSource' && index.isRelative(source)) ||
        !index.isAbsolute(id));
}
...
function generateAssetFileName(name, source, originalFileName, sourceHash, outputOptions, bundle) {
    ...
    return makeUnique(renderNamePattern(typeof outputOptions.assetFileNames === 'function'
        ...
        ext: () => path$n.extname(emittedName).slice(1),
        extname: () => path$n.extname(emittedName),
        hash: size => sourceHash.slice(0, Math.max(0, size || DEFAULT_HASH_SIZE)),
        name: () => emittedName.slice(0, Math.max(0, emittedName.length - path$n.extname(emittedName).length))
    }), bundle);
}
function reserveFileNameInBundle(fileName, { bundle }, log) {
    if (bundle[lowercaseBundleKeys].has(fileName.toLowerCase())) {
        log(index.LOGLEVEL_WARN, index.logFileNameConflict(fileName));
    }
    else {
...
function hasValidName(emittedFile) {
    ...
    return !validatedName || (typeof validatedName === 'string' && !index.isPathFragment(validatedName));
}
function getValidSource(source, emittedFile, fileReferenceId) {
    if (!(typeof source === 'string' || source instanceof Uint8Array)) {
        const assetName = emittedFile.fileName || emittedFile.name || fileReferenceId;
        return index.error(index.logFailedValidation(`Could not set source for ${typeof assetName === 'string' ? `asset "${assetName}"` : 'unnamed asset'}, asset source needs to be a string, Uint8Array or Buffer.`));
    }
    return source;
}
function getAssetFileName(file, referenceId) {
    if (typeof file.fileName !== 'string') {
        return index.error(index.logAssetNotFinalisedForFileName(file.name || referenceId));
    }
...
function getChunkFileName(file, facadeChunkByModule) {
    ...
    return index.error(index.logChunkNotGeneratedForFileName(file.fileName || file.name));
}
class FileEmitter {
    constructor(graph, options, baseFileEmitter) {
        ...
        this.emitFile = (emittedFile) => {
            if (!hasValidType(emittedFile)) {
                return index.error(index.logFailedValidation(`Emitted files must be of type "asset", "chunk" or "prebuilt-chunk", received "${emittedFile && emittedFile.type}".`));
            }
            if (emittedFile.type === 'prebuilt-chunk') {
                return this.emitPrebuiltChunk(emittedFile);
            }
            if (!hasValidName(emittedFile)) {
                return index.error(index.logFailedValidation(`The "fileName" or "name" properties of emitted chunks and assets must be strings that are neither absolute nor relative paths, received "${emittedFile.fileName || emittedFile.name}".`));
            }
        ...
        this.finaliseAssets = () => {
            for (const [referenceId, emittedFile] of this.filesByReferenceId) {
                if (emittedFile.type === 'asset' && typeof emittedFile.fileName !== 'string')
                    return index.error(index.logNoAssetSourceSet(emittedFile.name || referenceId));
            }
        };
        this.getFileName = (fileReferenceId) => {
            const emittedFile = this.filesByReferenceId.get(fileReferenceId);
            if (!emittedFile)
                return index.error(index.logFileReferenceIdNotFoundForFilename(fileReferenceId));
            if (emittedFile.type === 'chunk') {
        ...
        this.setAssetSource = (referenceId, requestedSource) => {
            ...
            if (!consumedFile)
                return index.error(index.logAssetReferenceIdNotFoundForSetSource(referenceId));
            if (consumedFile.type !== 'asset') {
                return index.error(index.logFailedValidation(`Asset sources can only be set for emitted assets but "${referenceId}" is an emitted chunk.`));
            }
            if (consumedFile.source !== undefined) {
                return index.error(index.logAssetSourceAlreadySet(consumedFile.name || referenceId));
            }
    ...
    emitChunk(emittedChunk) {
        if (this.graph.phase > BuildPhase.LOAD_AND_PARSE) {
            return index.error(index.logInvalidRollupPhaseForChunkEmission());
        }
        if (typeof emittedChunk.id !== 'string') {
            return index.error(index.logFailedValidation(`Emitted chunks need to have a valid string id, received "${emittedChunk.id}"`));
        }
        const consumedChunk = {
    ...
    emitPrebuiltChunk(emitPrebuiltChunk) {
        if (typeof emitPrebuiltChunk.code !== 'string') {
            return index.error(index.logFailedValidation(`Emitted prebuilt chunks need to have a valid string code, received "${emitPrebuiltChunk.code}".`));
        }
        if (typeof emitPrebuiltChunk.fileName !== 'string' ||
            index.isPathFragment(emitPrebuiltChunk.fileName)) {
            return index.error(index.logFailedValidation(`The "fileName" property of emitted prebuilt chunks must be strings that are neither absolute nor relative paths, received "${emitPrebuiltChunk.fileName}".`));
        }
...
function getLogHandler(level, code, logger, pluginName, logLevel) {
    if (index.logLevelPriority[level] < index.logLevelPriority[logLevel]) {
        return doNothing;
    }
    return (log, pos) => {
        if (pos != null) {
            logger(index.LOGLEVEL_WARN, index.logInvalidLogPosition(pluginName));
        }
        log = normalizeLog(log);
...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    return {
        ...
        cache: cacheInstance,
        debug: getLogHandler(index.LOGLEVEL_DEBUG, 'PLUGIN_LOG', onLog, plugin.name, logLevel),
        emitFile: fileEmitter.emitFile.bind(fileEmitter),
        error(error_) {
            return index.error(index.logPluginError(normalizeLog(error_), plugin.name));
        },
        getFileName: fileEmitter.getFileName,
        getModuleIds: () => graph.modulesById.keys(),
        getModuleInfo: graph.getModuleInfo,
        getWatchFiles: () => Object.keys(graph.watchFiles),
        info: getLogHandler(index.LOGLEVEL_INFO, 'PLUGIN_LOG', onLog, plugin.name, logLevel),
        load(resolvedId) {
        ...
        parse: index.parseAst,
        resolve(source, importer, { attributes, custom, isEntry, skipSelf } = index.BLANK) {
            skipSelf ??= true;
            return graph.moduleLoader.resolveId(source, importer, custom, isEntry, attributes || index.EMPTY_OBJECT, skipSelf ? [{ importer, plugin, source }] : null);
        },
        setAssetSource: fileEmitter.setAssetSource,
        warn: getLogHandler(index.LOGLEVEL_WARN, 'PLUGIN_WARNING', onLog, plugin.name, logLevel)
    };
}
...
class PluginDriver {
        ...
        if (basePluginDriver) {
            for (const plugin of userPlugins) {
                for (const hook of inputHooks) {
                    if (hook in plugin) {
                        options.onLog(index.LOGLEVEL_WARN, index.logInputHookInOutputPlugin(plugin.name, hook));
                    }
                }
            }
        }
    ...
    runHook(hookName, parameters, plugin, replaceContext) {
        ...
        return Promise.resolve()
            ...
            return index.error(index.logPluginError(error_, plugin.name, { hook: hookName }));
        });
    }
    /**
...
function validateFunctionPluginHandler(handler, hookName, plugin) {
    if (typeof handler !== 'function') {
        index.error(index.logInvalidFunctionPluginHook(hookName, plugin.name));
    }
}
function validateAddonPluginHandler(handler, hookName, plugin) {
    if (typeof handler !== 'string' && typeof handler !== 'function') {
        return index.error(index.logInvalidAddonPluginHook(hookName, plugin.name));
    }
}
function noReturn() { }
...
class Graph {
        ...
        for (const module of this.implicitEntryModules) {
            for (const dependant of module.implicitlyLoadedAfter) {
                if (!(dependant.info.isEntry || dependant.isIncluded())) {
                    index.error(index.logImplicitDependantIsNotIncluded(dependant));
                }
            }
        }
    }
    sortModules() {
        ...
        for (const cyclePath of cyclePaths) {
            this.options.onLog(index.LOGLEVEL_WARN, index.logCircularDependency(cyclePath));
        }
        ...
        for (const module of this.modules) {
            for (const importDescription of module.importDescriptions.values()) {
                if (importDescription.name !== '*' &&
                    ...
                    module.log(index.LOGLEVEL_WARN, index.logMissingExport(importDescription.name, module.id, importDescription.module.id), importDescription.start);
                }
            }
        }
    }
...
function getLogger(plugins, onLog, watchMode, logLevel) {
    ...
    const minimalPriority = index.logLevelPriority[logLevel];
    const logger = (level, log, skipped = index.EMPTY_SET) => {
        index.augmentLogMessage(log);
        const logPriority = index.logLevelPriority[level];
        ...
        for (const plugin of plugins) {
            ...
            if (handler.call({
                debug: getLogHandler(index.LOGLEVEL_DEBUG),
                error: (log) => index.error(normalizeLog(log)),
                info: getLogHandler(index.LOGLEVEL_INFO),
                meta: { rollupVersion: version, watchMode },
                warn: getLogHandler(index.LOGLEVEL_WARN)
            }, level, log) === false) {
                return;
            }
        }
...
async function normalizeInputOptions(config, watchMode) {
    ...
    const logLevel = config.logLevel || index.LOGLEVEL_INFO;
...
const getModuleContext = (config, context) => {
    ...
    if (configModuleContext) {
        ...
        for (const [key, moduleContext] of Object.entries(configModuleContext)) {
            contextByModuleId[path$n.resolve(key)] = moduleContext;
        }
...
const getTreeshake = (config) => {
    ...
    const configWithPreset = getOptionWithPreset(config.treeshake, treeshakePresets, 'treeshake', index.URL_TREESHAKE, 'false, true, ');
    return {
        annotations: configWithPreset.annotations !== false,
        correctVarValueBeforeDeclaration: configWithPreset.correctVarValueBeforeDeclaration === true,
        manualPureFunctions: configWithPreset.manualPureFunctions ?? index.EMPTY_ARRAY,
...
const getHasModuleSideEffects = (moduleSideEffectsOption) => {
    ...
    if (moduleSideEffectsOption) {
        index.error(index.logInvalidOption('treeshake.moduleSideEffects', index.URL_TREESHAKE_MODULESIDEEFFECTS, 'please use one of false, "no-external", a function or an array'));
    }
    return () => true;
};
...
const getFile = (config, preserveModules, inputOptions) => {
    ...
    if (typeof file === 'string') {
        if (preserveModules) {
            return index.error(index.logInvalidOption('output.file', index.URL_OUTPUT_DIR, 'you must set "output.dir" instead of "output.file" when using the "output.preserveModules" option'));
        }
        if (!Array.isArray(inputOptions.input))
            return index.error(index.logInvalidOption('output.file', index.URL_OUTPUT_DIR, 'you must set "output.dir" instead of "output.file" when providing named inputs'));
    }
    return file;
};
const getFormat = (config) => {
    ...
    switch (configFormat) {
        ...
        default: {
            return index.error(index.logInvalidOption('output.format', index.URL_OUTPUT_FORMAT, `Valid values are "amd", "cjs", "system", "es", "iife" or "umd"`, configFormat));
        }
    }
};
const getInlineDynamicImports = (config, inputOptions) => {
    ...
    if (inlineDynamicImports && (Array.isArray(input) ? input : Object.keys(input)).length > 1) {
        return index.error(index.logInvalidOption('output.inlineDynamicImports', index.URL_OUTPUT_INLINEDYNAMICIMPORTS, 'multiple inputs are not supported when "output.inlineDynamicImports" is true'));
    }
...
const getPreserveModules = (config, inlineDynamicImports, inputOptions) => {
    ...
    if (preserveModules) {
        if (inlineDynamicImports) {
            return index.error(index.logInvalidOption('output.inlineDynamicImports', index.URL_OUTPUT_INLINEDYNAMICIMPORTS, `this option is not supported for "output.preserveModules"`));
        }
        if (inputOptions.preserveEntrySignatures === false) {
            return index.error(index.logInvalidOption('preserveEntrySignatures', index.URL_PRESERVEENTRYSIGNATURES, 'setting this option to false is not supported for "output.preserveModules"'));
        }
    }
...
const getPreserveModulesRoot = (config) => {
    ...
    return path$n.resolve(preserveModulesRoot);
};
const getAmd = (config) => {
    const mergedOption = {
        autoId: false,
    ...
    if ((mergedOption.autoId || mergedOption.basePath) && mergedOption.id) {
        return index.error(index.logInvalidOption('output.amd.id', index.URL_OUTPUT_AMD_ID, 'this option cannot be used together with "output.amd.autoId"/"output.amd.basePath"'));
    }
    if (mergedOption.basePath && !mergedOption.autoId) {
        return index.error(index.logInvalidOption('output.amd.basePath', index.URL_OUTPUT_AMD_BASEPATH, 'this option only works with "output.amd.autoId"'));
    }
...
const getDir = (config, file) => {
    ...
    if (typeof dir === 'string' && typeof file === 'string') {
        return index.error(index.logInvalidOption('output.dir', index.URL_OUTPUT_DIR, 'you must set either "output.file" for a single-file build or "output.dir" when generating multiple chunks'));
    }
    return dir;
};
...
function getExports(config, unsetOptions) {
    ...
    if (configExports == null) {
        ...
        return index.error(index.logInvalidExportOptionValue(configExports));
    }
    return configExports || 'auto';
}
const getExternalImportAttributes = (config, inputOptions) => {
    if (config.externalImportAssertions != undefined) {
        index.warnDeprecation(`The "output.externalImportAssertions" option is deprecated. Use the "output.externalImportAttributes" option instead.`, index.URL_OUTPUT_EXTERNALIMPORTATTRIBUTES, true, inputOptions);
    }
    return config.externalImportAttributes ?? config.externalImportAssertions ?? true;
};
const getGeneratedCode = (config) => {
    const configWithPreset = getOptionWithPreset(config.generatedCode, generatedCodePresets, 'output.generatedCode', index.URL_OUTPUT_GENERATEDCODE, '');
    return {
...
const validateInterop = (interop) => {
    if (!ALLOWED_INTEROP_TYPES.has(interop)) {
        return index.error(index.logInvalidOption('output.interop', index.URL_OUTPUT_INTEROP, 
        // eslint-disable-next-line unicorn/prefer-spread
...
const getManualChunks = (config, inlineDynamicImports, preserveModules) => {
    ...
    if (configManualChunks) {
        if (inlineDynamicImports) {
            return index.error(index.logInvalidOption('output.manualChunks', index.URL_OUTPUT_MANUALCHUNKS, 'this option is not supported for "output.inlineDynamicImports"'));
        }
        if (preserveModules) {
            return index.error(index.logInvalidOption('output.manualChunks', index.URL_OUTPUT_MANUALCHUNKS, 'this option is not supported for "output.preserveModules"'));
        }
    }
...
const getSourcemapBaseUrl = (config) => {
    ...
    if (sourcemapBaseUrl) {
        if (index.isValidUrl(sourcemapBaseUrl)) {
            return index.addTrailingSlashIfMissed(sourcemapBaseUrl);
        }
        return index.error(index.logInvalidOption('output.sourcemapBaseUrl', index.URL_OUTPUT_SOURCEMAPBASEURL, `must be a valid URL, received ${JSON.stringify(sourcemapBaseUrl)}`));
    }
};

function rollup$1(rawInputOptions) {
...
async function rollupInternal(rawInputOptions, watcher) {
    ...
    const result = {
        ...
        async generate(rawOutputOptions) {
            if (result.closed)
                return index.error(index.logAlreadyClosed());
...
async function getProcessedInputOptions(inputOptions, watchMode) {
    ...
    const logLevel = inputOptions.logLevel || index.LOGLEVEL_INFO;
    ...
    for (const plugin of plugins) {
        ...
        const processedOptions = await handler.call({
            debug: getLogHandler(index.LOGLEVEL_DEBUG, 'PLUGIN_LOG', logger, name, logLevel),
            error: (error_) => index.error(index.logPluginError(normalizeLog(error_), name, { hook: 'onLog' })),
            info: getLogHandler(index.LOGLEVEL_INFO, 'PLUGIN_LOG', logger, name, logLevel),
            meta: { rollupVersion: version, watchMode },
            warn: getLogHandler(index.LOGLEVEL_WARN, 'PLUGIN_WARNING', logger, name, logLevel)
        }, inputOptions);
        if (processedOptions) {
    ...
    return catchUnfinishedHookActions(outputPluginDriver, async () => {
        ...
        if (isWrite) {
            ...
            if (!outputOptions.dir && !outputOptions.file) {
                return index.error(index.logMissingFileOrDirOption());
            }
...
function getOutputOptions(inputOptions, unsetInputOptions, rawOutputOptions, outputPluginDriver) {
    return normalizeOutputOptions(outputPluginDriver.hookReduceArg0Sync('outputOptions', [rawOutputOptions], (outputOptions, result) => result || outputOptions, pluginContext => {
        const emitError = () => pluginContext.error(index.logCannotEmitFromOptionsHook());
        return {
            ...pluginContext,
            emitFile: emitError,
...
async function writeOutputFile(outputFile, outputOptions) {
    const fileName = path$n.resolve(outputOptions.dir || path$n.dirname(outputOptions.file), outputFile.fileName);
    // 'recursive: true' does not throw if the folder structure, or parts of it, already exist
    await fsp.mkdir(path$n.dirname(fileName), { recursive: true });
    return fsp.writeFile(fileName, outputFile.type === 'asset' ? outputFile.source : outputFile.code);
}
...
const isCompatibleTerminal =
  require$$0$3__namespace && require$$0$3__namespace.isatty && require$$0$3__namespace.isatty(1) && env.TERM && !isDumbTerminal;
...
function handleError(error, recover = false) {
    ...
    if (error.loc) {
        outputLines.push(`${index.relativeId((error.loc.file || error.id))} (${error.loc.line}:${error.loc.column})`);
    }
    else if (error.id) {
        outputLines.push(index.relativeId(error.id));
    }
    if (error.frame) {
...
function mergeInputOptions(config, overrides, plugins, log, onLog) {
    ...
    const inputOptions = {
        ...
        treeshake: getObjectOption(config, overrides, 'treeshake', objectifyOptionWithPresets(treeshakePresets, 'treeshake', index.URL_TREESHAKE, 'false, true, ')),
...
async function mergeOutputOptions(config, overrides, log) {
    ...
    const outputOptions = {
        ...
        generatedCode: getObjectOption(config, overrides, 'generatedCode', objectifyOptionWithPresets(generatedCodePresets, 'output.generatedCode', index.URL_OUTPUT_GENERATEDCODE, '')),
...
async function watchInternal(configs, emitter) {
    ...
    if (watchOptionsList.length === 0) {
        return index.error(index.logInvalidOption('watch', index.URL_WATCH, 'there must be at least one config where "watch" is not set to "false"'));
    }
    await loadFsEvents();
    const { Watcher } = await Promise.resolve().then(function () { return require('./watch-DjI0XaN8.cjs'); });
...
var rollup = /*#__PURE__*/Object.freeze({
  __proto__: null,
  VERSION: version,
  rollup: rollup$1,
  watch: watch
